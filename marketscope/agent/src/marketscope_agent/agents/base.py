...
from marketscope_common.constants import ConfidenceLevel
from marketscope_common.exceptions import (
    AgentExecutionError,
    LLMCallError,
    MCPToolCallError,
    OutputParsingError,
...
from marketscope_common.logging import get_agent_logger
...
class BaseAgent(ABC, Generic[T]):
    ...
    async def call_llm(
        ...
        for attempt in range(self.max_retries + 1):
            try:
                ...
                from marketscope_common.monitoring.metrics import record_llm_usage
                record_llm_usage(model=effective_model, prompt_tokens=pt, completion_tokens=ct)

                # 비용 계산
                from marketscope_common.monitoring._pricing import calculate_cost
    ...
    async def run(self, state: dict[str, Any]) -> dict[str, Any]:
        ...
        try:
            ...
            # Prometheus 메트릭
            from marketscope_common.monitoring.metrics import observe_agent_execution
            ...
            from marketscope_common.monitoring.metrics import inc_agent_error
