...
async def get_floating_population(
    ...
    Returns hourly breakdown, gender ratio, age distribution, and daily average.
    """
    ...
    async with async_session() as session:
        ...
        hourly_stmt = (
            select(
                FPModel.time_slot,
                func.sum(FPModel.total_pop).label("total"),
            )
            .where(
                FPModel.district_code == district_code,
                FPModel.quarter == quarter,
            )
        ...
        gender_row = (await session.execute(gender_stmt)).one()
