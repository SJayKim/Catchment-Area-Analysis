...
> 작성일: 2026-03-19 | 통합 정리: 2026-03-23

---

## 목차

1. [Phase 1 MVP 기준 — MCP 서버 활성 상태](#phase-1-mvp-기준--mcp-서버-활성-상태)
2. [공통 사항](#공통-사항)
   - [MCP 서버 구현 패턴](#mcp-서버-구현-패턴)
   - [에이전트의 도구 발견 및 호출 흐름](#에이전트의-도구-발견-및-호출-흐름)
   - [인증 및 API 키 관리](#인증-및-api-키-관리)
   - [Rate Limiting 미들웨어](#rate-limiting-미들웨어)
   - [요청/응답 로깅](#요청응답-로깅)
   - [공용 에러 클래스](#공용-에러-클래스)
3. [MCP Server #1: 공공데이터 (Public Data)](#mcp-server-1-공공데이터-public-data)
4. [MCP Server #2: 지도/GIS (Maps)](#mcp-server-2-지도gis-maps)
5. [MCP Server #3: 부동산 (Real Estate)](#mcp-server-3-부동산-real-estate)
6. [MCP Server #4: 뉴스/SNS (News & Social)](#mcp-server-4-뉴스sns-news--social)
7. [MCP Server #5: 규제/인허가 (Regulatory)](#mcp-server-5-규제인허가-regulatory)
8. [MCP Server #6: 금융/대출 (Finance)](#mcp-server-6-금융대출-finance)
9. [MCP Server #7: Database (PostGIS 공간쿼리)](#mcp-server-7-database-postgis-공간쿼리)
10. [MCP Server #8: Google Maps](#mcp-server-8-google-maps)
11. [MCP Server #9: Naver Maps](#mcp-server-9-naver-maps)
12. [부록: 전체 도구 요약표](#부록-전체-도구-요약표)
...
| `DATA_API_NAVER_CLIENT_ID` | 네이버 Cloud Platform / Search API (Naver Maps Server) | Naver Cloud Platform | ✅ 필수 |
| `DATA_API_NAVER_CLIENT_SECRET` | 네이버 Cloud Platform / Search API (Naver Maps Server) | Naver Cloud Platform | ✅ 필수 |
| `DATA_API_GOOGLE_MAPS_KEY` | Google Maps Platform API (Google Maps Server) | Google Cloud | ✅ 필수 |
| `DATABASE_URL` | PostgreSQL + PostGIS 연결 (Database Server) | 자체 인프라 | ✅ 필수 |
...
| Google Maps | Google Maps Platform | QPS 50 (기본) | max_tokens=50, refill_rate=50 | ✅ 활성 |
| Naver Maps | NCP Geocoding/ReverseGeo | 3,000건/일, QPS 10 | max_requests=10, window_seconds=1 | ✅ 활성 |
| Naver Maps | NCP Directions | 1,000건/일, QPS 5 | max_requests=5, window_seconds=1 | ✅ 활성 |
| Naver Maps | Naver Search Local | 25,000건/일, QPS 10 | max_requests=10, window_seconds=1 | ✅ 활성 |
| Naver Maps | NCP Static Map | 3,000건/일, QPS 10 | max_requests=10, window_seconds=1 | ✅ 활성 |
...
## MCP Server #7: Database (PostGIS 공간쿼리)
...
#### 7.2.1 `database.query_nearby_stores`
...
#### 7.2.2 `database.query_population_grid`
...
#### 7.2.3 `database.query_revenue_by_area`
...
#### 7.2.4 `database.query_commercial_district`
...
#### 7.2.5 `database.execute_spatial_query`
...
#### 8.3.2 `google_maps.reverse_geocode`
...
#### 8.3.3 `google_maps.nearby_search`
...
#### 8.3.4 `google_maps.place_details`
...
#### 9.2.1 Naver Cloud Platform API (지도)
...
#### 9.3.2 `naver_maps.reverse_geocode`
...
| 38 | Database | `database.query_nearby_stores` | PostGIS ST_DWithin | 1시간 | ✅ 활성 | 입지/경쟁/유동인구 |
| 39 | Database | `database.query_population_grid` | population_grid 테이블 | 6시간 | ✅ 활성 | 입지/경쟁/유동인구 |
| 40 | Database | `database.query_revenue_by_area` | area_revenue 테이블 | 6시간 | ✅ 활성 | 입지/경쟁/유동인구 |
| 41 | Database | `database.query_commercial_district` | commercial_districts 테이블 | 12시간 | ✅ 활성 | 입지/경쟁/유동인구 |
| 42 | Database | `database.execute_spatial_query` | 전체 DB (SELECT only) | 없음 | ✅ 활성 | 입지/경쟁/유동인구 |
| 43 | Google Maps | `google_maps.geocode` | Google Geocoding API | - | ✅ 활성 | 입지/경쟁 |
| 44 | Google Maps | `google_maps.reverse_geocode` | Google Geocoding API | - | ✅ 활성 | 입지/경쟁 |
| 45 | Google Maps | `google_maps.nearby_search` | Google Places API | - | ✅ 활성 | 입지/경쟁 |
| 46 | Google Maps | `google_maps.place_details` | Google Place Details API | - | ✅ 활성 | 입지/경쟁 |
| 47 | Google Maps | `google_maps.directions` | Google Directions API | - | ✅ 활성 | 입지/경쟁 |
| 48 | Naver Maps | `naver_maps.geocode` | NCP Geocoding | - | ✅ 활성 | 입지/트렌드 |
| 49 | Naver Maps | `naver_maps.reverse_geocode` | NCP Reverse Geocoding | - | ✅ 활성 | 입지/트렌드 |
| 50 | Naver Maps | `naver_maps.local_search` | Naver Search Local | - | ✅ 활성 | 입지/트렌드 |
| 51 | Naver Maps | `naver_maps.directions` | NCP Directions | - | ✅ 활성 | 입지/트렌드 |
| 52 | Naver Maps | `naver_maps.static_map` | NCP Static Map | - | ✅ 활성 | 입지/트렌드 |
