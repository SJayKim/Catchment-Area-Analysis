...
| 현재 구현 | `get_store_history` Tool + RiskCard (안정성 게이지 + 생존기간 차트) |

> ⚠ `store_history` 테이블에 실데이터 없음 — data.go.kr API 미연동. Real 모드에서는 `stores` 개폐업 집계로 파생 계산.
...
- [x] "이 자리 위험해?" → RiskCard 생성
- [x] 안정성 점수 0~100 표시
- [x] 업종별 평균 생존 기간 바 차트
- [x] 위험 업종 경고 표시
- [x] 분기별 개폐업 추이 차트
- [ ] "안전한 업종 추천" → F07 링크 (UI 직접 연결 미완)
