...
- [x] 챗봇에서 "A랑 B 비교해줘" → CompareCard 생성 (Planner comparison intent)
- [x] 지도 비교모드에서 2~3개 상권 선택 시 자동 비교
- [x] 비교표에 주요 지표 정확 표시
- [x] AI 종합 의견 데이터 기반 생성
- [x] 비교 상권이 각각 다른 색상(파랑/빨강/초록)으로 지도 하이라이트 — 2026-04-17 `DistrictLayer.tsx` 수정
- [x] 최대 3개 제한 (`compareList` in `districtStore`)
