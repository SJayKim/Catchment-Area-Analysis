...
아래 흐름도는 Planner가 계획을 세운 뒤 Actor가 Tool을 꺼내 쓰고, 결과가 카드 이벤트로 변환되는 전체 흐름을 보여 준다.
...
아래 표는 각 도구 파일의 `@register_tool(...)` 인수를 직접 확인해 작성했다. `agent.md` §4의 표와 대조 결과, **카드 타입 항목에서 차이**가 있다 — agent.md는 `get_floating_population` · `get_estimated_sales` · `get_store_info` · `get_population_info` 의 card 타입을 `population` / `sales` / `store` 등으로 기재하지만, **실제 코드에서 이 4종은 `card_type=None`으로 등록**되어 있다(카드 직접 발행 없이 다른 도구에 데이터를 공급하는 역할). 코드 실젯값을 우선한다.
...
3. **의존성 Layer 병렬 실행으로 응답을 빠르게 한다** — Planner의 `ToolPlanStep.depends_on` 필드로 DAG를 구성하고, 같은 Layer의 도구들은 `asyncio.gather`로 동시에 실행된다. `get_district_summary` 내부도 같은 전략으로 4종 병렬 호출을 사용한다.

4. **내부 헬퍼(`get_district_benchmarks`)는 Planner plan에 등장하지 않는다** — `@register_tool` 미등록 함수는 레지스트리에 없으므로 Planner가 계획 단계에서 직접 선택할 수 없다. 이는 의도된 설계다 — 헬퍼는 다른 도구 내부에서만 호출되며, 외부에 단독 노출되지 않는다.
