...
이 학습 문서들(00~06 · ARCHITECTURE-MAP)의 모든 Mermaid 도식은 아래 **공통 팔레트**를 사용합니다. 어느 문서를 펼치든 같은 색은 같은 계층을 뜻합니다.
...
3. **Trust Kernel — 검증-후-방출** — 답변의 모든 수치는 도구 반환값(또는 `compute` 파생값)에 ±5% 이내로 바인딩되어야 한다. 바인딩 안 되는 수치는 교정 → 마스킹 → 결정론적 대체 순으로 집행된다. → 04
