...
| 서울 밖 거절 | `abstain` 메타툴 1급 거부 (+ chat.py 상권 감지의 `STRONG_TOP1_MIN=0.70` 오매칭 가드) | `loop/tools_fc.py` · `chat.py` + `agent/utils/entity_matching.py` |
...
| F02 AI 챗봇 | `chat/ChatPanel.tsx` + `chatStore` | `agent/runtime.py` → `loop/engine.py` (v2 루프) |
...
#   ⚠ Mock(LLM_PROVIDER=mock)에서는 에이전트가 레거시 경로(graph.py)를 탄다 (v2 는 실제 LLM 필요)
#   백엔드
