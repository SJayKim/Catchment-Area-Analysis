...
```mermaid
flowchart TD
    Tool["AI Agent Tool\n(예: get_floating_population)"]
    DA["DataAccess 파사드\n(main.py lifespan에서 조립)"]
...
`recommend_business`의 핵심 점수 계산식은 다음과 같다 (L166):
...
```python
# server/server/repositories/real/recommendation.py, L14-25
...
실제 사용 예시는 `recommendation.py` L139에서 볼 수 있다:
...
3. **데이터 정확성**:
   - **store floor** (`MIN_RELIABLE_STORES=3`): 점포 수가 적은 카테고리는 점포당 매출 지표가 통계적으로 부풀려지므로 랭킹에서 제외한다. 전부 영세하면 fallback + 안내 메시지.
