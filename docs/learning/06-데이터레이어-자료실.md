...
> 선행 문서: [05 도구 12종: 전문 계측기](05-도구12종-전문계측기.md) · 다음: [00 시작하기로 돌아가기](00-시작하기.md)
...
**후속 보강 — 점수 유계 밴드 (2026-07)**: 플로어와 별개로, 정규화 점수가 0~100 극단에 붙는 것도 신뢰 문제였다(1위가 항상 "100점"으로 보이면 과신을 유도한다). 현재는 `_band_scores`(L24)가 raw_score 순위를 보존한 채 **55~95 범위**(`SCORE_BAND_MIN/MAX`, L18-19)로 정규화한다 — "만점 추천"이 구조적으로 불가능하다.
