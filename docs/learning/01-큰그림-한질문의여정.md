...
요리가 홀에 오는 방식도 특별하다. 진행 상황이 실시간으로 전해지는 것이다. "질문 분석 중"(thinking) → "데이터 수집 중"(tool) → "카드 완성"(card) → **"요리 진행률 안내방송"**("응답 작성 중... 37%" — 주방장이 마지막 요리를 만드는 동안 홀 스피커가 진행률을 알린다) → "최종 설명"(text) → "이런 것도 물어봐요"(suggestion) → "다 됐어요"(done). 손님은 요리가 완성되기 전에 이미 중간 결과(카드)와 진행률을 받아 본다.

여기서 눈여겨볼 점: 요리 본체는 **위생 검사(팩트체크)를 통과하기 전에는 절대 홀로 새어 나가지 않는다**. 주방장이 쓰는 문장이 한 글자씩 그대로 흘러나오는 게 아니라, 완성된 원고가 검사를 통과한 뒤에야 한 번에 나간다. 그 사이의 침묵을 진행률 안내방송이 메운다 — 이것이 뒤에서 배울 **옵션 B(최종 응답 스트리밍)** 다.
...
sequenceDiagram
    ...
    loop 최대 6 모델 턴 (budget: 도구 12회 · 90s)
        엔진->>LLM: ainvoke/astream_with_fallback(메시지 + 도구 스키마 12종)
        ...
        opt 최종 답변 작성 중 (옵션 B — astream 버퍼링)
            LLM-->>엔진: 응답 청크 (버퍼에만 쌓임 — 본문 미방출)
            FastAPI-->>브라우저: SSE: thinking {"step": "응답 작성 중... 37%"}
        end
    end
...
`agent_loop_version`(기본 `"v2"`, config.py:86)이면서 mock 프로바이더가 아니면 v2 루프(`agent/loop/engine.py`)다. (레거시 각주: mock 모드는 가짜 LLM이 tool-call 을 못 해 레거시 경로(`graph.py`)를 탄다.)

**② 루프 본체 — `agent/loop/engine.py`**: 시스템 프롬프트 + 최근 히스토리 6턴 + 질문으로 대화를 조립하고, 도구 스키마 12종을 바인딩해 모델 턴을 돈다.

```python
# server/server/agent/loop/engine.py:193-200 (run_agent 발췌)
...
| 최종 답변 작성 중 (옵션 B) | `thinking` ("응답 작성 중... 37%") | 진행률 라벨 실시간 갱신 (버퍼링 중 침묵 메움) |
...
2. **SSE 이벤트의 순서**: `map_cmd` → `thinking` → `tool`/`card`/`tool_end` (반복) → `thinking`(결과 분석 → "응답 작성 중 n%" → 필요 시 수치 검증) → `text`(90자 청크) → `suggestion` → `done`. 이 순서가 사용자가 보는 점진적 로딩 경험을 만든다. 진행률도 새 이벤트 타입이 아니라 `thinking` 재사용 — 이벤트 계약은 v2 7종 그대로다. (프론트 타입 정의는 10종 — v2 미발생 `plan` 방어 코드와 프론트 측 `error` 포함.)
...
4. **검증-후-방출 + 진행률로 침묵 메우기**: 답변의 모든 수치는 Trust Kernel의 바인딩 검사(±5%)를 통과해야 방출된다. `text` 가 토큰 스트림이 아니라 90자 청크인 이유 — 이미 완성·검증된 텍스트를 잘라 보내는 것이다. 대신 작성 중의 침묵은 옵션 B가 `thinking`("응답 작성 중... n%")으로 메운다: 모델 출력을 astream 으로 받아 **버퍼에만 쌓고**, 분량 기반 진행률만 내보낸다(롤백 `AGENT_LOOP_STREAM_FINAL=false`). 검증 원칙을 포기하지 않고 체감 대기만 줄인 절충이다. → [04 §6](04-AI에이전트-v2루프.md)
...
7. **LLM 폴백 체인**: 역할별 모델 분담 대신 **단일 tool-calling 모델 + per-invoke 폴백**(Claude Sonnet → GPT → Gemini pro → flash, `LLM_PROVIDER` 가 선호 프로바이더를 맨 앞으로 승격)이다. 모델 ID는 전부 설정(env)에서 오므로 은퇴 모델도 배포 없이 교체한다. 상세는 [04 §5](04-AI에이전트-v2루프.md).
