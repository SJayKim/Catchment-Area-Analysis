...
- **디렉토리별 가이드**: 프론트 작업은 [frontend/CLAUDE.md](frontend/CLAUDE.md), 백엔드/에이전트 작업은 [server/CLAUDE.md](server/CLAUDE.md) (Claude Code 가 작업 디렉토리별 자동 로드).
...
├── frontend/       # Next.js 14 (src/app, components, stores, hooks, lib) → frontend/CLAUDE.md
├── server/         # FastAPI (server/server/* 안에 실제 패키지) → server/CLAUDE.md
...
- **SSE 포맷**: `event:` 라인 없이 `data: {json}` 안에 type 임베드 — 표준 SSE 파서 금지 ([frontend/CLAUDE.md](frontend/CLAUDE.md)).
...
Global `~/.claude/settings.json` 훅이 도구 실패 교훈을 auto memory 에 저장. 규칙:
